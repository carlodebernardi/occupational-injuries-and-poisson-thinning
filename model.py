...
y_rep = np.array(az.extract(idata2, var_names="y_rep"))
...
plt.xlabel('Severity classes')
plt.ylabel('Reported cases (log)')
plt.savefig('figs/m2_metal.png', dpi=140, bbox_inches='tight')
plt.show()
plt.close()
...
plt.ylabel('Number of injuries')
plt.xlabel('Subsectors')
plt.savefig('figs/m2_sc3.png', dpi=140, bbox_inches='tight')
plt.show()
plt.close()
...
idata3 = az.from_cmdstanpy(fit)

s3 = az.summary(idata3, var_names=["alpha","sh","sh_sigma","sigma_incidence","log_incidence"])
s3.to_csv('figs/m3_summary.csv')

az.plot_trace(idata3, var_names=['alpha'])
plt.show()
plt.close()

az.plot_trace(idata3, var_names=['sh'])
plt.show()
plt.close()

az.plot_trace(idata3, var_names=['sh_sigma'])
plt.show()
plt.close()

az.plot_trace(idata3, var_names=['log_incidence'])
plt.show()
plt.close()

##

suf = az.summary(idata3, var_names=["log_incidence"])
suf['sector'] = (suf.index.str.split('[').str[1]
                          .str.split(',').str[0].astype(int))

selected_sectors = np.array([1, 7, 13, 16, 19, 24])-1
x = np.arange(data['K'])

for i,s in enumerate(selected_sectors):
    su = suf[suf.sector == s]
    off = i / (selected_sectors.shape[0] + 2)
    plt.scatter(x+off, np.exp(su['mean']), color=f'C{s}', label=s+1)
    plt.plot([x+off,x+off], np.exp([su['hdi_3%'], su['hdi_97%']]), color=f'C{s}')
plt.xticks(x, labels=x+1)
plt.grid(alpha=0.3)
plt.yscale('log')
plt.title('Incidence by severity, selected sectors')
plt.xlabel('Severity class')
plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.savefig('figs/m3_incid_sec.png', dpi=140, bbox_inches='tight')
plt.show()
plt.close()

# ##

y_rep = np.array(az.extract(idata3, var_names="y_rep"))
...
plt.ylabel('Number of injuries')
plt.xlabel('Subsectors')
plt.savefig('figs/m3_sc3.png', dpi=140, bbox_inches='tight')
plt.show()
plt.close()